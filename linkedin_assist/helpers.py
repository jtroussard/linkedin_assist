...
def compare_and_keep(current, saved_file_name):
    ...
    :param saved_file_name: file name of saved_guids. Saved GUIDS represent past GUIDS which were known
...
def make_suggestions(keepers, inp, limits):
...
def update_records(file, records, job, date):
    """Updates the record file. This file keeps track of each job posted. Specifically noting the last date it was
    posted and how many total times the job was posted.
    ...
    for element in records:
        if element["guid"] == job["guid"]:
            ...
            file.truncate()
    if not ele_found:
        records.append({"guid": job["guid"], "last_post": str(date)[:10], "counter": 1})
        file.seek(0)
        json.dump(records, file, indent=2)
        file.truncate()
    return True
...
def create_message(job, message_bank):
    """Selects a random message template from the message bank and then populates the necessary blanks with values from
    the job posting.

    :param job: Job post data.
    :type job: dict
    :param message_bank: Collection of message templates.
    :type message_bank: list
...
def apply_config_selections(config, list_of_jobs):
    """When the program run in AUTO mode the user must configure the program to determine how many posts it will be
    allowed to post. For example if the program finds 100 viable posts, but the user configured the program to only
    post 3 posts at a time, 3 random posts of the 100 will be posted to LinkedIn. Since the posts have already been
    filterd on days from last post and total posts, eventually all posts will be posted. Another example would include
    a low return value. Say the program only finds 1 viable post however the user configured the porgram to post 5 at a
    time. The Program will only post the one available job.
